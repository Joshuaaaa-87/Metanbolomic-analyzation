...
#
# Analysis version:
#   Single_KEGG + Multiple_KEGG all expanded
#
# Meaning:
#   If one feature maps to multiple KEGG IDs,
#   all KEGG IDs are kept for enrichment analysis.
#
# Warning:
#   This may inflate pathway hits because one LC-MS feature
#   can contribute multiple candidate KEGG IDs.
#
# Input:
...
#
# raw_data_path:
#   你的 metabolomic reanalysis 主資料夾。
#
# input_path:
#   前面整理好的 KEGG-mapped measurement data 所在位置。
#
# output_path:
#   這次 Single + Multiple 全展開 KEGG enrichment 結果會存在這裡。
...
# 這一步在做什麼？
# 讀入已經整理好的 KEGG-mapped measurement data。
#
# 這一步得到的 Data 代表什麼？
# NEG_measure_KEGG:
...
#----4. expand all KEGG IDs from Single_KEGG and Multiple_KEGG features----
# 這一步在做什麼？
# 1. Single_KEGG features 只有一個 KEGG_ID，保留。
# 2. Multiple_KEGG features 有多個 KEGG_ID，用 ; 分隔。
# 3. 這一步把所有 KEGG_ID 全部拆開成多列。
#
# 這一步得到的 Data 代表什麼？
# measure_expanded_KEGG:
#   每一列是一個 Feature_ID - KEGG_ID pair。
#   如果同一個 feature 對到 5 個 KEGG ID，這裡會變成 5 列。
#
# 注意：
# 這版會保留所有 candidate KEGG IDs。
# 因此 pathway hit 可能被放大，適合作為 exploratory analysis。

measure_expanded_KEGG <- measure_KEGG_all %>%
  ...
  filter(!is.na(KEGG_ID)) %>%
  distinct(Feature_ID, KEGG_ID, .keep_all = TRUE) %>%
  mutate(
    KEGG_analysis_strategy = case_when(
      KEGG_mapping_status == "Single_KEGG" ~ "Single_KEGG",
      KEGG_mapping_status == "Multiple_KEGG" ~ "Expanded_Multiple_KEGG",
      TRUE ~ "Other"
    )
  )
...
write_csv(
  measure_expanded_KEGG,
  file.path(output_path, "1_Expanded_KEGG_measurement_for_enrichment.csv")
)

measure_expanded_KEGG_summary <- measure_expanded_KEGG %>%
  group_by(Mode, KEGG_mapping_status, KEGG_analysis_strategy) %>%
  summarise(
    Feature_KEGG_rows = n(),
    Unique_features = n_distinct(Feature_ID),
    Unique_KEGG_IDs = n_distinct(KEGG_ID),
    .groups = "drop"
  )

write_csv(
  measure_expanded_KEGG_summary,
  file.path(output_path, "1_Expanded_KEGG_measurement_Summary.csv")
)
...
stopifnot(all(sample_cols %in% colnames(measure_expanded_KEGG)))


#----6. calculate differential metabolites: each condition vs DMSO----
# 這一步在做什麼？
# 對每個 Feature_ID - KEGG_ID pair，計算各死亡條件相對 DMSO 的：
# 1. DMSO mean
# 2. Condition mean
# 3. log2FC
# 4. p value
...
#
# 注意：
# 如果同一個 feature 對到多個 KEGG ID，
# 這幾個 KEGG rows 會有相同 intensity / log2FC / p value。
# 這是 expanded KEGG 分析的特性。
...
write_csv(
  ...
  file.path(output_path, "2_Differential_metabolites_Expanded_KEGG_vs_DMSO.csv")
)
...
# 這一步在做什麼？
# 篩選每個死亡條件中，相對 DMSO 上升的 Feature_ID - KEGG_ID pairs。
#
# 這一步得到的 Data 代表什麼？
# high_up_metabolites_p005:
#   log2FC >= 1 且 p_value < 0.05 的 expanded KEGG rows。
...
write_csv(
  high_up_metabolites_p005,
  file.path(output_path, "3_High_up_metabolites_log2FC1_p005_Expanded_KEGG.csv")
)

high_up_metabolites_p005_summary <- high_up_metabolites_p005 %>%
  group_by(Condition, Mode, KEGG_analysis_strategy) %>%
  summarise(
    Feature_KEGG_rows = n(),
    Unique_features = n_distinct(Feature_ID),
    Unique_KEGG_IDs = n_distinct(KEGG_ID),
...
# 這一步在做什麼？
# 篩選更嚴格的 high-up Feature_ID - KEGG_ID pairs。
#
# 這一步得到的 Data 代表什麼？
# high_up_metabolites_FDR01:
#   log2FC >= 1 且 FDR < 0.1 的 expanded KEGG rows。
...
write_csv(
  high_up_metabolites_FDR01,
  file.path(output_path, "3_High_up_metabolites_log2FC1_FDR01_Expanded_KEGG.csv")
)
...
#
# background:
#   所有 expanded KEGG IDs。
#
# input:
#   每個死亡條件 high-up metabolites 的 expanded KEGG IDs。
#
# 注意：
# 這裡用 unique KEGG_ID 做 enrichment。
# 也就是同一個 KEGG_ID 即使來自多個 features，只算一個 KEGG compound。
# 但 Multiple_KEGG 的不同 candidate KEGG IDs 會都被納入。

kegg_background <- measure_expanded_KEGG %>%
...
write_csv(
  kegg_background,
  file.path(output_path, "4_KEGG_background_Expanded_KEGG.csv")
)

write_csv(
  kegg_input_by_condition_p005,
  file.path(output_path, "4_KEGG_input_by_condition_log2FC1_p005_Expanded_KEGG.csv")
)

write_csv(
  kegg_input_by_condition_FDR01,
  file.path(output_path, "4_KEGG_input_by_condition_log2FC1_FDR01_Expanded_KEGG.csv")
)
...
#   path:map00230
#
# 內部 enrichment 使用 path:mapXXXXX。
# 輸出顯示轉成 path:hsaXXXXX。
...
run_enrichment_by_condition <- function(kegg_input_by_condition, output_prefix) {
  ...
  if (nrow(kegg_input_by_condition) == 0) {
    ...
    write_csv(
      empty_result,
      file.path(output_path, paste0("6_KEGG_enrichment_", output_prefix, "_Expanded_KEGG.csv"))
    )
  ...
  write_csv(
    top20_kegg_pathways,
    file.path(output_path, paste0("7_Top20_KEGG_pathways_by_condition_", output_prefix, "_Expanded_KEGG.csv"))
  )
  ...
  write_csv(
    ...
    file.path(output_path, paste0("8_Concise_KEGG_pathway_table_", output_prefix, "_Expanded_KEGG.csv"))
  )
