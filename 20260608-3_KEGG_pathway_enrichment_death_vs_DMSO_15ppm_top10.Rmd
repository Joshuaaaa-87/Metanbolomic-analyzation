...
#----0. load packages----
# 這一步在做什麼？
# 載入資料整理、統計與 KEGG REST 讀取需要的 packages。
...
#
# output_path:
#   KEGG enrichment 結果會存在這裡。
#
# 注意：
# 這版會重新建立並覆蓋 output_path 裡同名檔案。
...
#----4. keep Single_KEGG features for main enrichment----
# 這一步在做什麼？
# 只保留 Single_KEGG features 作為正式 KEGG enrichment 主分析。
#
# 為什麼？
# Multiple_KEGG 代表一個 LC-MS feature 對到多個 KEGG ID。
# 如果全部拆開，會把同一個訊號重複計算，造成 pathway hit 膨脹。
#
# 這一步得到的 Data 代表什麼？
# measure_single_KEGG:
#   每個 feature 只對到一個 KEGG_ID 的乾淨主分析資料。

measure_single_KEGG <- measure_KEGG_all %>%
  filter(KEGG_mapping_status == "Single_KEGG") %>%
  filter(!is.na(KEGG_ID)) %>%
  mutate(
    KEGG_ID = str_squish(as.character(KEGG_ID)),
...
write_csv(
  measure_single_KEGG,
  file.path(output_path, "1_Single_KEGG_measurement_for_enrichment.csv")
)
...
stopifnot(all(sample_cols %in% colnames(measure_single_KEGG)))
...
write_csv(
  differential_metabolite_table,
  file.path(output_path, "2_Differential_metabolites_Single_KEGG_vs_DMSO.csv")
)
...
write_csv(
  high_up_metabolites_p005,
  file.path(output_path, "3_High_up_metabolites_log2FC1_p005_Single_KEGG.csv")
)

high_up_metabolites_p005_summary <- high_up_metabolites_p005 %>%
  count(Condition, Mode)
...
write_csv(
  high_up_metabolites_FDR01,
  file.path(output_path, "3_High_up_metabolites_log2FC1_FDR01_Single_KEGG.csv")
)
...
#
# background:
#   所有 Single_KEGG features 中的 KEGG IDs。
#
# input:
...
kegg_background <- measure_single_KEGG %>%
...
write_csv(
  kegg_background,
  file.path(output_path, "4_KEGG_background_Single_KEGG.csv")
)

write_csv(
  kegg_input_by_condition_p005,
  file.path(output_path, "4_KEGG_input_by_condition_log2FC1_p005_Single_KEGG.csv")
)

write_csv(
  kegg_input_by_condition_FDR01,
  file.path(output_path, "4_KEGG_input_by_condition_log2FC1_FDR01_Single_KEGG.csv")
)
...
run_enrichment_by_condition <- function(kegg_input_by_condition, output_prefix) {
  ...
  if (nrow(kegg_input_by_condition) == 0) {
    ...
    write_csv(
      empty_result,
      file.path(output_path, paste0("6_KEGG_enrichment_", output_prefix, "_Single_KEGG.csv"))
    )
  ...
  write_csv(
    top20_kegg_pathways,
    file.path(output_path, paste0("7_Top20_KEGG_pathways_by_condition_", output_prefix, "_Single_KEGG.csv"))
  )
  ...
  write_csv(
    ...
    file.path(output_path, paste0("8_Concise_KEGG_pathway_table_", output_prefix, ".csv"))
  )
...
#
# 注意：
# 如果 high_up_metabolites_FDR01 很少，這一步可能結果也很少。
